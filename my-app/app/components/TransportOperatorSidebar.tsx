/**
 * Collapsible left navigation for the transport operator portal, mirroring
 * MillOwnerSidebar.tsx: a fixed nav list (no permission-based filtering)
 * since every transport operator sees the same links.
 */
"use client";
...
const NAV_ITEMS = [
  { label: "Dashboard", href: "/transport-operator", icon: LayoutDashboard },
  { label: "Fleet", href: "/transport-operator/fleet", icon: Truck },
  { label: "Messages", href: "/transport-operator/messages", icon: MessageSquare },
  { label: "Settings", href: "/transport-operator/settings", icon: Settings },
];

/** Renders the logo, collapse toggle, nav links (highlighting the active route), and a logout form. */
export default function TransportOperatorSidebar() {
