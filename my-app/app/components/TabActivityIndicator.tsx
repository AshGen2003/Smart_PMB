 ...
 * 2. Favicon becomes a spinning brand-colored ring while any real network
 *    activity is in flight. Page navigations, Server Action submits, and
 *    every apiFetch call to the backend all go through the browser's own
 *    fetch() under the hood — patching it once here catches all of them
 *    centrally, instead of wiring a "start/stop loading" call into every
 *    individual button and form across the app. Next.js's own background
 *    route prefetching (fetched ahead of a <Link> entering the viewport,
 *    not something the user actually clicked) is deliberately excluded
 *    via its next-router-prefetch header — otherwise the icon would spin
...
export default function TabActivityIndicator() {
  ...
  useEffect(() => {
    notifyActivityChange = setActive;
